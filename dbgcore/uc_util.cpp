...
bool uc_util::disasm(unsigned long long address, unsigned char *table, x86_disasm_context_type *context)
{
	return false;
}

unsigned long long uc_util::mnemonic_str(void *handle, unsigned long long address, unsigned long processor_bit, unsigned char *dump, char *output, size_t output_size)
{
	return 0;
}
...
unsigned long uc_util::trace(void *handle, unsigned long long ptr, dbg::util::code &b, bool is_range)
{
	return 0;
}

unsigned long uc_util::browse(void *handle, unsigned long long ptr, std::set<unsigned long long> &entry_point_set, bool is_safe)
{
	return 0;
}
...
void uc_util::code_callback(uc_engine *uc, uint64_t address, uint32_t size, void *user_data)
{
	...
	if (cd)
	{
		unsigned long err_code = UC_CODE_ERR;
...
void uc_util::rw_unmap_callback(uc_engine *uc, uc_mem_type type, uint64_t address, int size, int64_t value, void *user_data)
{
	...
	{
		err_code = UC_WRITE_ERR;
	}
	else if (type == UC_MEM_READ_UNMAPPED)
	{
		err_code = UC_READ_ERR;
	}
...
void uc_util::fetch_unmap_callback(uc_engine *uc, uc_mem_type type, uint64_t address, int size, int64_t value, void *user_data)
{
	...
	{
		unsigned long err_code = UC_FETCH_ERR;
		cd->cb(cd->api, cd->util, &err_code, address, cd->user_context);
	}
}

unsigned long uc_util::run(void *handle, analyze_callback_type cb, void *cb_context, unsigned long long base, unsigned long long end, std::set<unsigned long long> &entry_point_set)
{
	...
	uc_engine *uc = (uc_engine *)api->get_object(EMULATOR_HANDLE_ID);
	if (!uc)
	{
		return 0;
	}

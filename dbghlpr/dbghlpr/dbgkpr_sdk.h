#ifndef __DEFINE_DBGKPR_SDK__
#define __DEFINE_DBGKPR_SDK__
...
#define CreateRemoteApiHandle()				dbg::linker::api::create(dbg::CORE_IID::IID_REMOTEAPI_CORE)
	...
	namespace linker
	{
		namespace api
		{
			api_handle create(unsigned long id);

			bool open(api_handle handle, char *path);
			bool open(api_handle handle, unsigned long pid);

			bool close(api_handle handle);
			bool get_thread_id_list(api_handle handle, std::list<unsigned long> &tid_list);

			bool select_thread(api_handle handle, unsigned long tid);
			__declspec(dllexport) unsigned long current_thread_id(api_handle handle);

			void * virtual_alloc(api_handle handle, unsigned long long base, unsigned long size, unsigned long allocation_type, unsigned long protect_type);
			int virtual_free(api_handle handle, void *ptr, unsigned long size, unsigned long free_type);

			bool query_virtual_memory(api_handle handle, unsigned long long virtual_address, void *out_memory_info);
			unsigned long __stdcall read_virtual_memory(api_handle handle, unsigned long long virtual_address, unsigned char *out_memory, unsigned long read_size);
			unsigned long __stdcall write_virtual_memory(api_handle handle, unsigned long long virtual_address, unsigned char *input_memory, unsigned long write_size);

			bool __stdcall get_thread_context(api_handle handle, cpu_context_type *context);
			bool __stdcall set_thread_context(api_handle handle, cpu_context_type *context);

			bool get_symbol_name(api_handle handle, unsigned long long offset, char *buffer, unsigned long size_of_buffer, unsigned long *size_of_name, unsigned long long *disp);
			bool stack_trace(api_handle handle, unsigned long tid, dbg::api::stack_frame_type_ptr stack_frame, unsigned long size_of_stack_frame, unsigned long *stack_count);

			bool write_dump(api_handle handle, char *path);

			bool suspend(api_handle api_handle, unsigned long tid);
			bool resume(api_handle api_handle, unsigned long tid);
		}

		namespace util
		{
			util_handle create(unsigned long id);

			bool disasm(util_handle handle, unsigned long long address, unsigned char *table, dbg::PROCESSOR_TYPE type, dbg::util::x86_disasm_context_type *context);
			unsigned long long mnemonic_str(util_handle handle, void *api_handle, unsigned long long address, unsigned char *dump, dbg::PROCESSOR_TYPE type, char *output, size_t output_size);
			bool find_reference_value(util_handle handle, api_handle api_handle, unsigned long long base, unsigned long long size, dbg::PROCESSOR_TYPE type, std::multimap<unsigned long long, unsigned long long> &ref_map);

			unsigned long trace(util_handle handle, void *api_handle, unsigned long long ptr, dbg::PROCESSOR_TYPE type, dbg::util::code &b, bool is_safe);
			unsigned long browse(util_handle handle, void *api_handle, unsigned long long ptr, dbg::PROCESSOR_TYPE type, std::set<unsigned long long> &entry_point_set, bool is_safe);
			unsigned long run(util_handle handle, void *api_handle, dbg::util::analyze_callback_type cb, void *cb_context, unsigned long long base, unsigned long long end, dbg::PROCESSOR_TYPE type, std::set<unsigned long long> &entry_point_set);
		}
	}
}

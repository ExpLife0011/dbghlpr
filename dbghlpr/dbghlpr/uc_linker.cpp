...
void *uc_linker::get_object(unsigned long id)
{
	...
	switch (id)
	{
	case dbg::OBJECT_ID::EMULATOR_HANDLE_ID :
		i = (void *)uc_;
...
{
	if (bit == dbg::PROCESSOR_TYPE::X86_PROCESSOR_BIT_TYPE)
	{
		mode_ = UC_MODE_32;
	}
	else
...
unsigned long uc_linker::current_thread_id()
{
	return 0;
}
...
bool uc_linker::suspend(unsigned long tid)
{
	return false;
}

bool uc_linker::resume(unsigned long tid)
{
	return false;
}